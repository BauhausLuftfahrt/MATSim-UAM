...
public class UAMDepartureHandlerOld implements DepartureHandler {
	...
	public UAMDepartureHandlerOld(UAMManager uamManager,
	...
	public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> linkId) {
		...
		switch (mode) {
			case "access_uam":
				...
				if (vehicle == null || destStation == null) {
					...
					double walkSpeed = ((PlansCalcRouteConfigGroup) scenario.getConfig().getModules().
				...
				double beelineDistanceFactor = ((PlansCalcRouteConfigGroup) scenario.getConfig().getModules().
						get("planscalcroute")).getBeelineDistanceFactors().get("walk");
				double travelTime = distance * beelineDistanceFactor / walkSpeed != 0.0 ? distance * beelineDistanceFactor / walkSpeed : 1.0;
			...
			case "uam":
				...
				//Coord destCoord = network.getLinks().get(leg.getRoute().getEndLinkId()).getCoord();
			...
			case "egress_uam":
				...
				double beelineDistanceFactorEgress = ((PlansCalcRouteConfigGroup) scenario.getConfig().getModules().
						get("planscalcroute")).getBeelineDistanceFactors().get("walk");
				double travelTimeEgress = distanceEgress * beelineDistanceFactorEgress / walkSpeedEgress != 0.0 ? distanceEgress * beelineDistanceFactorEgress / walkSpeedEgress : 1.0;
		...
		//we did not handle the departure, we just addapt the plan, we will let the teleportationEngine to handle the departure
		return false;
	}
