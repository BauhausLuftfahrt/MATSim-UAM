...
public class UAMSingleRideAppender {
	...
	public void schedule(UAMRequest request, UAMVehicle vehicle, double now) {
		...
		Future<Path> pickup = router.calcLeastCostPath(stayTask.getLink().getToNode(), // pickup path is from downstream node of stayTask Link to upstream node from request origin Link.
				request.getFromLink().getFromNode(), now, null, null);
		Future<Path> dropoff = router.calcLeastCostPath(request.getFromLink().getToNode(), // dropoff path is from downstream node of request origin link to upstream node from request destination Link.
