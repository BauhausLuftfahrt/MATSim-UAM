...
/**
 * This script generates a Generates a csv file containing information about UAM Stations.
...
public class ConvertUAMStationsFromUAMVehicles {
	...
	static public void extract(String network, String uamVehicles, String outfile) throws IOException {
		...
		for (UAMStation station : uamStations.getUAMStations().values()) {
			uamData.add(new UAMStationItem(station.getName(),
					station.getId(),
					station.getLandingCapacity(),
					(int) station.getPreFlightTime(),
					(int) station.getPostFlightTime(),
					(int) station.getDefaultWaitTime(),
					station.getLocationLink().getId().toString()));
		}
